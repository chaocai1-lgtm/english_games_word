...
def render_sidebar(game: WordGame = None):
    ...
    with st.sidebar:
        ...
        if st.button("🏰 魔塔闯关", use_container_width=True):
            st.session_state.game_mode = "tower_select"
            st.rerun()
        
        if st.button("🌱 词根探索", use_container_width=True):
            st.session_state.game_mode = "root_explore"
            st.rerun()
        
        if st.button("📖 复习错题", use_container_width=True):
            st.session_state.game_mode = "review"
            st.rerun()
        
        if st.button("⏱️ 限时挑战", use_container_width=True):
            st.session_state.game_mode = "speed_challenge"
            st.rerun()
        
        if st.button("✍️ 拼写大师", use_container_width=True):
            st.session_state.game_mode = "spelling"
            st.rerun()
        
        if st.button("🎁 幸运抽奖", use_container_width=True):
            st.session_state.game_mode = "lucky_wheel"
            st.rerun()
        
        if st.button("🏅 成就殿堂", use_container_width=True):
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("👨‍👩‍👧 家长", use_container_width=True):
...
def render_tower_mode(game: WordGame):
    ...
    if st.session_state.question_index >= len(words):
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            if st.button("📋 返回楼层选择", use_container_width=True):
                st.session_state.game_mode = "tower_select"
                st.session_state.floor_words = []
                st.session_state.question_index = 0
                st.rerun()
        with col2:
            if floor < 9 and st.button("⬆️ 挑战下一层", use_container_width=True):
                st.session_state.current_floor = floor + 1
                st.session_state.floor_words = []
                st.session_state.question_index = 0
                st.rerun()
        with col3:
            if st.button("🔄 重新挑战本层", use_container_width=True):
                st.session_state.floor_words = []
                st.session_state.question_index = 0
                st.rerun()
        return
...
def render_spelling_mode(game: WordGame):
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("💡 显示首字母", disabled=st.session_state.spelling_hint_used):
